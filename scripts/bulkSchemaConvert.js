 ...
 */
function updateTypesIndexFile(typesDir, interfaces) {
  ...
  for (const { interfaceName, interfaceFilename } of interfaces) {
    lines.push(
  ...
  log(`   ✅ Updated types index: ${indexPath}`, colors.green);
}
...
async function processFile(
  ...
  try {
    ...
    // Write files in parallel and await completion
    const writeOperations = [
      fs.promises.writeFile(
        path.join(typesDir, `${interfaceFilename}.ts`),
        typesCode,
      ),
      fs.promises.writeFile(
        path.join(schemasDir, `${baseFilename}.ts`),
        schemaCode,
      ),
...
async function main() {
  ...
  log("═══════════════════════════════════════════", colors.magenta);
  log(
    `✅ Successfully converted: ${results.success.length} files`,
    colors.green,
  );
